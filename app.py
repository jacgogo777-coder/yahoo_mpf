...
secret_default_stock = "2330.TW"
secret_warmup = 60
try:
    if hasattr(st, "secrets"):
        if "DEFAULT_STOCK" in st.secrets:
            secret_default_stock = str(st.secrets["DEFAULT_STOCK"])
        elif "MY_STOCK" in st.secrets:
            secret_default_stock = str(st.secrets["MY_STOCK"])
        if "WARMUP_DAYS" in st.secrets:
            secret_warmup = int(st.secrets["WARMUP_DAYS"])
except Exception:
    pass
